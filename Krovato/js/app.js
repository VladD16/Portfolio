 ...
 */
    ...
    (() => {
        ...
        const inputPhone = document.querySelector(".bottom-product-info__input");
        if (inputPhone) modules_flsModules.inputmask = Inputmask({
        ...
        function initSliders() {
            if (document.querySelector(".hero-page__slider")) new swiper_core_Swiper(".hero-page__slider", {
                ...
                autoplay: true,
                pagination: {
                    el: ".hero-page__swiper-pagination",
                    clickable: true
                },
                navigation: {
                    prevEl: ".hero-page__swiper-button-prev",
                    nextEl: ".hero-page__swiper-button-next"
                },
                on: {}
            });
            ...
            if (document.querySelector(".blog-page__slider")) new swiper_core_Swiper(".blog-page__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
            ...
            if (document.querySelector(".info-product__slider-mini")) new swiper_core_Swiper(".info-product__slider-mini", {
                ...
                on: {
                    click: function(swiper) {
                        ...
                        if (swiper.clickedIndex === lastVisibleIndex) swiper.slideNext();
                        if (swiper.clickedIndex === firstVisibleIndex) swiper.slidePrev();
                    }
                }
            });
        }
        ...
        window.onload = () => {
            ...
            if (window.innerWidth < maxWidth) {
                const firstSubmenu = document.querySelector(".submenu-catalog");
                const firstTitle = firstSubmenu.closest(".item-catalog").querySelector(".item-catalog__title");
                if (firstSubmenu) {
                    firstSubmenu.classList.add("open");
                    firstSubmenu.style.maxHeight = firstSubmenu.scrollHeight + "px";
                    firstTitle.classList.add("_spoller-active");
                    setTimeout((() => {
                        firstSubmenu.style.maxHeight = "none";
                    }), 500);
                }
                document.querySelectorAll(".item-catalog__button").forEach((button => {
                    button.addEventListener("click", (() => {
                        const currentSubmenu = button.closest(".item-catalog").querySelector(".submenu-catalog");
                        const currentTitle = button.closest(".item-catalog").querySelector(".item-catalog__title");
                        if (currentSubmenu.classList.contains("open")) {
                            ...
                            currentTitle.classList.remove("_spoller-active");
                        } else {
                            document.querySelectorAll(".submenu-catalog.open").forEach((submenu => {
                                submenu.style.maxHeight = submenu.scrollHeight + "px";
                                const otherTitle = submenu.closest(".item-catalog").querySelector(".item-catalog__title");
                                requestAnimationFrame((() => {
                                    submenu.style.maxHeight = "0";
                                }));
                                submenu.classList.remove("open");
                                otherTitle.classList.remove("_spoller-active");
                            }));
                            currentSubmenu.classList.add("open");
                            currentSubmenu.style.maxHeight = currentSubmenu.scrollHeight + "px";
                            currentTitle.classList.add("_spoller-active");
                            setTimeout((() => {
                                currentSubmenu.style.maxHeight = "none";
                            }), 500);
                        }
                    }));
                }));
                document.querySelectorAll(".submenu-catalog").forEach((submenu => {
                    submenu.style.maxHeight = "0";
                    submenu.style.overflow = "hidden";
                    submenu.style.transition = "max-height 0.5s ease-in-out";
                }));
                document.querySelectorAll(".submenu-catalog.open").forEach((submenu => {
                    submenu.style.maxHeight = "auto";
                }));
            }
            if (document.querySelector(".nav-page-reviews__item") && document.querySelector(".item-page-reviews")) {
                const tabButtons = document.querySelectorAll(".nav-page-reviews__item");
                const tabItems = document.querySelectorAll(".item-page-reviews");
                function updateAfter(element) {
                    const left = element.offsetLeft;
                    const width = element.offsetWidth;
                    document.documentElement.style.setProperty("--after-left", `${left}px`);
                ...
                tabButtons.forEach((button => {
                    button.addEventListener("click", (() => {
                        ...
                        tabItems.forEach((item => {
                            if (filter === "all" || item.getAttribute("data-category") === filter) item.style.display = "grid"; else item.style.display = "none";
                        }));
                    }));
                }));
                const activeButton = document.querySelector(".nav-page-reviews__item.active");
                if (activeButton) updateAfter(activeButton); else {
                    const firstButton = tabButtons[0];
                    firstButton.classList.add("active");
                    updateAfter(firstButton);
                }
                tabItems.forEach((item => {
                    setTimeout((() => {
            ...
            document.addEventListener("click", (e => {
                const targetElement = e.target;
                if (document.querySelector(".lang-header") && isTouchScreen) if (targetElement.closest(".lang-header")) document.documentElement.classList.toggle("lang-open"); else if (!targetElement.closest(".lang-header")) document.documentElement.classList.remove("lang-open");
                if (document.querySelector(".button-catalog")) if (targetElement.closest(".button-catalog")) {
                    document.documentElement.classList.toggle("catalog-open");
                    document.documentElement.classList.toggle("lock");
                    const catalogItems = document.querySelector(".items-catalog");
                    document.querySelector(":root").style.setProperty("--catalog-items-top", `${catalogItems.getBoundingClientRect().top + 20}px`);
                } else if (targetElement.closest(".item-catalog__link") || targetElement.closest(".submenu-catalog__link") || !targetElement.closest(".items-catalog")) {
                    document.documentElement.classList.remove("catalog-open");
                    document.documentElement.classList.remove("lock");
                }
                if (document.querySelector(".contacts-header__button-phone") || document.querySelector(".contacts-header__button-arrow")) if (targetElement.closest(".contacts-header__button-phone") || targetElement.closest(".contacts-header__button-arrow")) document.documentElement.classList.toggle("contacts-open"); else if (targetElement.closest(".contacts-header__main-phone") || targetElement.closest(".contacts-header__link") || targetElement.closest(".contacts-header__callback") || !targetElement.closest(".contacts-header")) document.documentElement.classList.remove("contacts-open");
                ...
                if (document.querySelector(".item-catalog__button")) {
                    ...
                    const minWidth = 768;
                    if (window.innerWidth >= minWidth) if (targetElement.closest(".item-catalog__button") && isTouchScreen) {
                        document.documentElement.classList.add("submenu-open");
                        catalogItems.forEach((item => {
                            if (item !== targetElement.closest(".item-catalog")) item.classList.remove("active"); else if (item === targetElement.closest(".item-catalog")) item.classList.add("active");
                        }));
                    } else if (!targetElement.closest(".item-catalog__button") && !targetElement.closest(".submenu-catalog")) {
            ...
            document.addEventListener("keydown", (e => {
                const targetElement = e.target;
                if (document.querySelector(".lang-header")) {
                    if (e.key === "Enter" && targetElement.closest(".lang-header")) document.documentElement.classList.toggle("lang-open");
                    if (e.key === "Escape") document.documentElement.classList.remove("lang-open");
                }
                if (document.querySelector(".button-catalog")) if (e.key === "Escape") {
                    document.documentElement.classList.remove("catalog-open");
                    document.documentElement.classList.remove("lock");
                }
                if (document.querySelector(".contacts-header__button-phone") || document.querySelector(".contacts-header__button-arrow")) if (e.key === "Escape") document.documentElement.classList.remove("contacts-open");
                if (document.querySelector(".checkbox__input")) if (e.key === "Enter") {
                    const checkboxes = document.querySelectorAll(".checkbox__input");
                    checkboxes.forEach((checkbox => {
                        if (targetElement === checkbox) checkbox.checked = !checkbox.checked;
                    }));
                }
            }));
        };
