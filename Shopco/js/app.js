    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                ...
                } else if (e.target.closest(".menu__link") || e.target.closest(".menu__sub-link") || !e.target.closest(".menu__body")) document.documentElement.classList.remove("menu-open");
            }));
        }
        ...
        function formRating() {
            ...
            function formRatingInit(rating, ratingSize) {
                ...
                for (let index = 0; index < ratingSize; index++) {
                    if (index === 0) ratingItems += `<div class="rating__items">`;
                    ratingItems += `\n        <label class="rating__item">\n          <input class="rating__input" type="radio" name="rating" value="${index + 1}">\n        </label>`;
                    if (index === ratingSize - 1) ratingItems += `</div>`;
                }
            ...
            function formRatingSet(rating, value) {
                ...
                const ratingParent = rating.closest(".rating");
                if (ratingParent) {
                    const ratingValueElement = ratingParent.querySelector(".rating__value");
                    if (ratingValueElement) {
                        let displayValue = value;
                        if (Number.isInteger(value)) displayValue = `${value}.0`;
                        ratingValueElement.innerHTML = `${displayValue}/<span>5</span>`;
                    }
                }
                ratingItems.forEach(((ratingItem, index) => {
                    ratingItem.classList.remove("rating__item--active");
                    if (ratingItem.querySelector("span")) ratingItem.querySelector("span").remove();
        ...
        function initSliders() {
            ...
            if (document.querySelector(".preview-slider")) new swiper_core_Swiper(".preview-slider", {
            ...
            if (document.querySelector(".main-slider")) new swiper_core_Swiper(".main-slider", {
                ...
                speed: 600,
                thumbs: {
                    swiper: document.querySelector(".preview-slider")
                }
            });
        }
        ...
        window.onload = () => {
            const headerBanner = document.querySelector(".banner-header");
            const headerMain = document.querySelector(".main-header");
            const updateHeight = () => {
                const headerBannerHeight = headerBanner ? headerBanner.getBoundingClientRect().height : 0;
                const headerMainHeight = headerMain ? headerMain.getBoundingClientRect().height : 0;
                const totalHeight = headerBannerHeight + headerMainHeight + "px";
            ...
            document.addEventListener("click", (e => {
                ...
                if (document.querySelector(".header__banner")) {
                    ...
                    if (targetEl.classList.contains("banner-header__close")) {
                        banner.style.display = "none";
                        updateHeight();
                    }
                }
                if (document.querySelector(".actions-header__item--search")) if (targetEl.closest(".actions-header__item--search")) document.documentElement.classList.toggle("search-open"); else if (!targetEl.closest(".search-header")) document.documentElement.classList.remove("search-open");
                if (document.querySelector(".header-categoty__filter-button")) if (targetEl.closest(".header-categoty__filter-button")) {
                    document.documentElement.classList.add("filter-open");
                    const filter = document.querySelector(".filter-category");
                    document.querySelector(":root").style.setProperty("--filter-top", `${filter.getBoundingClientRect().top + 20}px`);
                } else if (!targetEl.closest(".filter-category") || targetEl.closest(".filter-category__close-button") || targetEl.closest(".filter-links__link") || targetEl.closest(".form-category__button")) document.documentElement.classList.remove("filter-open");
            }));
        };
        window["FLS"] = true;
        addLoadedClass();
